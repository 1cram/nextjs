...
import { getCourseBySlug, getAllCourses, textAreaToArray } from "@/lib/static-data"
import { getSafeImageUrl, decodeHtmlEntities } from "@/lib/api-helpers"
import { notFound } from "next/navigation"

const StaticContent = ({ content, className }: { content: string; className?: string }) => {
...
export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  ...
  const title = decodeHtmlEntities(course.title.rendered)
  const description = decodeHtmlEntities(course.excerpt.rendered)
    .substring(0, 160)
    .replace(/<[^>]*>/g, "")
  const canonicalUrl = `/corsi/${course.slug}`
  ...
  return {
    ...
    twitter: {
      ...
      title,
      description,
      images: [course.featured_image_url],
    },
  }
}
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <TextReveal delay={0.2}>
                <StaticContent content={course.excerpt.rendered} className="prose prose-lg mb-8" />
              </TextReveal>
        ...
        <div className="max-w-7xl mx-auto">
          <div className="grid md:grid-cols-3 gap-12">
            <div className="md:col-span-2">
              ...
              <StaticContent content={course.content.rendered} className="prose prose-lg max-w-none" />
